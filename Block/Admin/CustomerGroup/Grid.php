...
class Grid extends CoreGrid
{
    ...
    {
        $url = $this->getUrl()->getUrl('form','admin\customerGroup',['id'=>$row->group_id]);
        return "mage.setUrl('{$url}').resetParam().load();";
    }

    public function getDeleteUrl($row)
    {
        $url = $this->getUrl()->getUrl('delete','admin\customerGroup',['id'=>$row->group_id]);
        return "mage.setUrl('{$url}').resetParam().load();";
    }

    public function getaddFilterUrl()
	{
        $url = $this->getUrl()->getUrl('filter','admin\customerGroup',null);
		return "mage.setUrl('{$url}').resetParam().setForm('#filterData').load();";
    }

    public function getaddNewUrl()
	{
		$url = $this->getUrl()->getUrl('form','admin\customerGroup',null);
		return "mage.setUrl('{$url}').resetParam().load();";
    }

    public function getChangeStatusUrl($row)
	{
		$url = $this->getUrl()->getUrl('changeStatus','admin\customerGroup',['id'=>$row->group_id,'status'=>$row->status]);
		return "mage.setUrl('{$url}').resetParam().load();";
	}
    ...
    {
        $cgroups = Mage::getModel("Model\CustomerGroup");
    ...
    public function pagination()
    {
        ...
        $cgroup = Mage::getModel('Model\CustomerGroup');

        $records = $cgroup->getAdapter()->fetchOne($query);
